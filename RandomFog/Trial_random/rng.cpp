...
void RNG::generateBatch(void)
{
	...
	if(cudaResult != CURAND_STATUS_SUCCESS)
	{
...
float RNG::getNextU01(void)
{
	...
	else
	{
		...
		unsigned int drawwOffset = index / m_qrngDimensions;
		return m_h_samples[dimOffset + drawwOffset];
	}
}
...
void RNG::resetSeed(void)
{
	...
	if (curandResult != CURAND_STATUS_SUCCESS)
	{
		std::string msg("Could not set pseudo-random generator seed:");
		msg += curandResult;
		throw runtime_error(msg);
	}
...
void RNG::updateDimensions(void)
{
	...
	if (curandResult != CURAND_STATUS_SUCCESS)
	{
		std::string msg("Could not set scrambles quasi-random number generator offset:");
		msg += curandResult;
		throw runtime_error(msg);
	}
